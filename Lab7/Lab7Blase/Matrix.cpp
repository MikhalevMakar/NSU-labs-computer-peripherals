...
Matrix operator*(const Matrix& first, const Matrix& second) {
    ...
    cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, first.sizeMatrix, first.sizeMatrix, first.sizeMatrix, 1.0, first.matrix, result.sizeMatrix, second.matrix, result.sizeMatrix, 0.0, result.matrix, result.sizeMatrix);
    return result;
}
