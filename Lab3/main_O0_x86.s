...
.L3:
        ...
        fildl   -20(%ebp) //Команда fild преобразует знаковый целочисленны  й операнд-источник в формат временного вещественного и помещает его в регистровый стек FPU для i
...
.L6:
        movl    4(%eax), %eax  //достаем значение N из 4(%eax) = argv[0]
        addl    $4, %eax //eax = eax + 4 // перешли 4(%eax) = N = *argv[1]
        movl    (%eax), %eax //разыменование указателя argv[1] = N
...
выровненные упакованные значения с плавающей запятой одинарной точности и
